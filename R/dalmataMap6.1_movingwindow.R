...
dalmataMap<-function (fn_srt,
  ...
  if (!is.null(fn_movingWindow_dim)){
    ...
    nxCells=(dimXRaster-fn_movingWindow_overlap*fn_movingWindow_dim)/(fn_movingWindow_dim-fn_movingWindow_overlap*fn_movingWindow_dim)
    nxCells=dimXRaster/nxCells
    nxCells<-floor(nxCells)
    qx=(dimXRaster-nxCells*fn_movingWindow_dim)/((1-nxCells)*fn_movingWindow_dim)

    nyCells=(dimYRaster-fn_movingWindow_overlap*fn_movingWindow_dim)/(fn_movingWindow_dim-fn_movingWindow_overlap*fn_movingWindow_dim)
    nyCells<-ceiling(nyCells)
    qy=(dimYRaster-nyCells*fn_movingWindow_dim)/((1-nyCells)*fn_movingWindow_dim)

    if (fn_verbose) cat('Moving window :\n X= ',nxCells,' / ',qx,' Y= ',nyCells,' / ',qy )

    xOrigin<-seq(extRaster[1],dimXRaster-fn_movingWindow_dim,length.out=nxCells)
    yOrigin<-seq(extRaster[3],dimYRaster-fn_movingWindow_dim,length.out=nyCells)
    ...
    xyDF<-lapply(1:nrow(xyDF),function(i){
      x<-xyDF[i,]
      out<-(c(x,
              c(x[1],x[2]+fn_movingWindow_dim),
              x+fn_movingWindow_dim,
              c(x[1]+fn_movingWindow_dim,x[2]),
              x))
...
#
    ...
    xOverlap<-c(rbind(xOrigin[-1],xOrigin[-length(xOrigin)]+fn_movingWindow_dim))
    yOverlap<-c(rbind(yOrigin[-1],yOrigin[-length(yOrigin)]+fn_movingWindow_dim))
...
browser()
    ...
    # NXO<-(1:length(xOverlap))[c(T,F)]
    # NYO<-(1:length(xOverlap))[c(T,F)]
    #
    # smallSquares<-lapply(NXO,function(x){
    #   out<-lapply(NYO,function(y){
    #     sf::st_polygon(list(matrix(c(xOverlap[x],yOverlap[y],
    #                                xOverlap[x],yOverlap[y+1],
    #                                xOverlap[x+1],yOverlap[y+1],
    #                                xOverlap[x+1],yOverlap[y],
    #                                xOverlap[x],yOverlap[y]),ncol=2,byrow = T)))
    #   })
    #   out<-sf::st_sfc(out)
    # })
    #
    # smallSquares<-do.call(c,smallSquares)
    #
    # xFullLength<-c(xOrigin,extRaster[2])
    # yFullLength<-c(yOrigin,extRaster[4])
    #
    #
    #
    #
    #
    # windowOverlap<-sf::st_intersection(sf::st_union(windowGrid),windowGrid)
    # windowOverlap<-windowOverlap[sf::st_geometry_type(windowOverlap)=='POLYGON']
  ...
  MULTIOUT<-parallel::parLapply(cl,newSrt,function(XSRT){
    raster::raster()
...
browser()
MULTIOUT_TOP<-lapply(MULTIOUT_TEMP,function(MO){
...
browser()
  ...
  # newBorders<-sapply(newBorders,function(x)if (length(x)==0) F else T,simplify = T,USE.NAMES = F)
  ...
  crossers<-sf::st_crosses(newBorders,MULTIOUT_TOP[[MTO]])
...
crossers<-sf::st_crosses(MULTIOUT_TOP,overlapTrelis)
crossers<-sapply(crossers,function(x)if (length(x)==0) F else T,simplify = T,USE.NAMES = F)
MULTIOUT_TOP[!crossers,'area']<-1
MULTIOUT_1<-MULTIOUT_TOP[!crossers,'area']
plot(MULTIOUT_1)
plot(MULTIOUT_TOP['area'],add=T)
plot(overlapTrelis)
  return(MULTIOUT)
  }
