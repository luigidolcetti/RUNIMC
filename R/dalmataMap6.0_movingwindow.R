...
dalmataMap<-function (fn_srt,
  ...
  if (!is.null(fn_movingWindow_dim)){
    ...
    dimXRaster<-extRaster[2]-extRaster[1]
    dimYRaster<-extRaster[4]-extRaster[3]
    ...
    xOrigin<-seq(0,dimXRaster-fn_movingWindow_dim,length.out=nxCells)
    yOrigin<-seq(0,dimYRaster-fn_movingWindow_dim,length.out=nxCells)
  ...
  print(cl)
  parallel::clusterExport(cl,c('newSrt',
                               'pixelBrake',
                               'fn_verbose',
                               'maxii',
                               'drct',
  ...
  MULTIOUT<-parallel::parLapply(cl,seq_along(newSrt),function(IXSRT){
